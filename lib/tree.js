...
Tree.prototype = {
	...
	draw: function(hsl){
		...
		this.ctx.save();
			...
			this.drawBranch(this.start_length,this.start_width, 0, hsl);

			this.ctx.save();
				this.ctx.translate(0,-this.start_length);
				this.drawBranches(1, hsl);
			this.ctx.restore();

		this.ctx.restore();
	},
	drawBranch: function(length, width, order, hsl){
	...
	drawBranches: function(order, hsl){
		...
		this.ctx.save();
			...
			this.drawBranch(new_length, new_width, order, hsl);

			this.ctx.save();
				this.ctx.translate(0,-new_length);
				this.drawBranches(order+1, hsl);
			this.ctx.restore();
